...
func (s *Service) Plugins() ([]PluginSummary, error) {
	...
	for _, root := range roots {
		...
		for _, entry := range entries {
			...
			summary := PluginSummary{
				PluginIdentitySummary: PluginIdentitySummary{ID: id},
				PluginStateSummary: PluginStateSummary{
					HasSignatureFile: inspection.HasSignatureFile,
				},
				PluginLocationSummary: PluginLocationSummary{
					PluginRoot:        entryDir,
			...
			if manifest, err := pluginmanifest.ParseStarlarkManifest(inspection.ManifestBytes); err == nil {
	...
	for id, summary := range outByID {
		...
		if bundleDir := bundleDirsByID[id]; bundleDir != "" {
			state, _ := marketplace.SignatureStateForDir(context.Background(), bundleDir, s.Config.Plugins.TrustedKeysFile, s.TrustedSigners)
