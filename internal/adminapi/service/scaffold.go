...
func (s *Service) ScaffoldPlugin(req PluginScaffoldRequest) (PluginScaffoldResponse, error) {
	...
	manifest := pluginmanifest.StarlarkManifest{
		Schema: pluginmanifest.StarlarkManifestSchema, ID: id, Name: name, Version: version, Entrypoint: pluginmanifest.StarlarkEntrypoint,
		Permissions: pluginmanifest.StarlarkManifestPermissions{Storage: req.Permissions.Storage, Discord: req.Permissions.Discord, Network: pluginmanifest.StarlarkManifestNetworkPermissions{HTTP: req.Permissions.Network.HTTP, Hosts: append([]string{}, req.NetworkHosts...)}, Automation: req.Permissions.Automation},
		Locales:     pluginmanifest.StarlarkManifestLocales{Default: locale, Supported: []string{locale}}, StateKeys: []string{}, Assets: []string{},
	}
	manifestBytes, err := json.MarshalIndent(manifest, "", "  ")
	if err != nil {
		return PluginScaffoldResponse{}, err
	}
	if _, err := pluginmanifest.ParseStarlarkManifest(manifestBytes); err != nil {
		return PluginScaffoldResponse{}, err
	}
...
`, messageID, commandName, commandDescription, descID)
	...
	files := []struct {
		...
		{rel: pluginmanifest.StarlarkEntrypoint, data: []byte(pluginStar)},
...
func (s *Service) SignPlugin(pluginID string) (string, error) {
	...
	privateKey, err := signing.ReadEd25519PrivateKeyFile(s.Config.Dashboard.SigningKeyFile)
	if err != nil {
		return "", err
	}
	sig, _, err := signing.SignDir(bundleDir, s.Config.Dashboard.SigningKeyID, privateKey)
	if err != nil {
		return "", err
	}
