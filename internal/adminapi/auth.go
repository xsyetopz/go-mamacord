...
func (s *Server) withOwner(next func(http.ResponseWriter, *http.Request, adminauth.Session)) http.HandlerFunc {
	return s.withAuth(func(w http.ResponseWriter, r *http.Request, sess adminauth.Session) {
...
func (s *Server) withCSRF(next func(http.ResponseWriter, *http.Request, adminauth.Session)) func(http.ResponseWriter, *http.Request, adminauth.Session) {
...
func (s *Server) handleMe(w http.ResponseWriter, r *http.Request) {
	sess, err := s.readSession(r)
	if err != nil {
		writeJSON(w, http.StatusOK, adminservice.SessionResponse{Authenticated: false})
		return
	}
	resp := adminservice.SessionResponse{
		Authenticated: true,
		IsOwner:       s.isOwnerUser(sess.UserID),
		CSRFToken:     sess.CSRFToken,
	}
	resp.User.ID = adminservice.Snowflake(sess.UserID)
...
func (s *Server) putSession(ctx context.Context, sess adminauth.Session) error {
	if s.sessions == nil {
...
func (s *Server) readSession(r *http.Request) (adminauth.Session, error) {
	...
	if !ok {
		return adminauth.Session{}, errors.New("invalid session")
	}
	if s.sessions == nil {
		return adminauth.Session{}, errors.New("invalid session")
	}
	...
	if time.Now().Unix() >= stored.ExpiresAt {
		...
		return adminauth.Session{}, errors.New("session expired")
	}
...
func (s *Server) currentOwnerStatus() adminservice.OwnerStatus {
	if s == nil || s.ownerStatus == nil {
		return adminservice.OwnerStatus{Source: "unresolved"}
	}
...
func avatarURL(user adminauth.OAuthUser) string {
