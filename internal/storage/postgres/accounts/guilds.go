...
ON CONFLICT(guild_id) DO UPDATE SET
	...
	guildIDDB, err := sqlvalue.Uint64ToInt64(g.GuildID, "guild_id")
	if err != nil {
		return err
	}
	ownerIDDB, err := sqlvalue.Uint64ToInt64(g.OwnerID, "owner_id")
	if err != nil {
		return err
	}
...
func (s guildStore) UpdateGuildOwner(ctx context.Context, guildID uint64, ownerID uint64, updatedAt time.Time) error {
	...
	ownerIDDB, err := sqlvalue.Uint64ToInt64(ownerID, "owner_id")
	if err != nil {
		return err
	}
