...
func (s pluginOAuthGrantStore) DeletePluginOAuthGrant(ctx context.Context, userID uint64, pluginID string) error {
	...
	if _, err := s.db.ExecContext(ctx, "DELETE FROM plugin_oauth_grants WHERE user_id = $1 AND plugin_id = $2", userID, pluginID); err != nil {
...
func (s pluginOAuthGrantStore) CountPluginOAuthGrants(ctx context.Context, userID uint64) (int, error) {
	...
	row := s.db.QueryRowContext(ctx, "SELECT COUNT(1) FROM plugin_oauth_grants WHERE user_id = $1", userID)
	var count int
