...
func Build(pluginID, localID string) (string, error) {
	if pluginID != strings.TrimSpace(pluginID) || localID != strings.TrimSpace(localID) || !pluginmanifest.ValidID(pluginID) || !customLocalIDPattern.MatchString(localID) {
...
func Parse(customID string) (string, string, bool) {
	...
	if !pluginmanifest.ValidID(pluginID) || !customLocalIDPattern.MatchString(localID) {
		return "", "", false
	}
	if strings.Contains(pluginID, ":") || strings.Contains(localID, ":") {
		return "", "", false
