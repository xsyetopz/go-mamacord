...
func (m *Host) LoadAll(ctx context.Context) error {
	...
	keys, err := signing.LoadTrustedKeys(ctx, m.trustedKeysFile, trustedSigners)
	if err != nil {
		return err
	}
...
func (m *Host) loadOne(
	...
	manifest, err := pluginmanifest.ParseStarlarkManifest(manifestBytes)
	if err != nil {
		return nil, nil, err
	}
	signaturePath := filepath.Join(bundleDir, "signature.json")
	var signature *signing.Signature
	if value, readErr := signing.ReadSignature(signaturePath); readErr == nil {
		signature = &value
	} else if !os.IsNotExist(readErr) {
		return nil, nil, readErr
	}
	if signature != nil {
		if err := signing.VerifyDirSignature(bundleDir, *signature, keys); err != nil {
			return nil, nil, err
		}
	} else if m.prodMode && !m.allowUnsignedPlugins {
		return nil, nil, errors.New("missing signature.json")
	}
	if err := pluginmanifest.ValidateBundleAuthority(bundleDir, manifest); err != nil {
		return nil, nil, err
	}
	...
	resources, err := pluginmanifest.ReadBundleResources(bundleDir, manifest)
	if err != nil {
		return nil, nil, err
	}
	...
	source, err := compile.OpenDirBundle(bundleDir)
	if err != nil {
		return nil, nil, err
	}
	...
	runtimeGeneration, err := generation.NewBuilder(printFn).Build(ctx, source, generationID)
	if err != nil {
		return nil, nil, err
	}
	manager, err := generation.NewGenerationManager(3*time.Second, func(retireErr error) {
		m.logger.Warn("plugin generation retirement", slog.String("plugin", manifest.ID), slog.String("err", retireErr.Error()))
	})
	if err != nil {
		return nil, nil, err
	}
	if err := manager.Activate(runtimeGeneration); err != nil {
		runtimeGeneration.BeginDrain()
		_ = runtimeGeneration.Release()
		return nil, nil, err
	}
	definition := runtimeGeneration.Definition()
	...
	commands, err := projection.CommandsFromContract(definition)
	if err != nil {
		closeCtx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
		_ = manager.Close(closeCtx)
		cancel()
	...
	events, jobs := projection.SubscriptionsFromContract(manifest.ID, definition)
	plugin := &Plugin{
		PluginSource: PluginSource{ID: manifest.ID, Dir: entryDir, BundleDir: bundleDir, Bundled: location.Bundled},
		PluginAuthority: PluginAuthority{
			Manifest: manifest, Signature: signature, Effective: effective, Capabilities: manifest.Capabilities(effective),
		},
		PluginCatalog: PluginCatalog{
			Resources: resources, Commands: commands, Events: events, Jobs: jobs, Definition: definition,
		},
		PluginExecution: PluginExecution{I18n: pluginI18n, Runtime: manager},
	}
...
func validateAutomationDefinitions(manifest pluginmanifest.StarlarkManifest, definition contract.Definition) error {
