	...
	for _, cmd := range cmds {
		...
		if _, exists := nextCommands[key]; exists {
			logger.WarnContext(
				...
				slog.String("type", projection.NormalizeCommandType(cmd.Command.Type)),
...
func commandLookupKey(kind, name string) string {
	return projection.NormalizeCommandType(kind) + ":" + strings.ToLower(strings.TrimSpace(name))
}
...
func (m *Host) Infos() []PluginInfo {
	...
	for _, pl := range m.plugins {
		...
		out = append(out, PluginInfo{
			...
			Commands:  append([]projection.Command(nil), pl.Commands...),
		})
	}
