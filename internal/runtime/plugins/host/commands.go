...
func commandToCreate(
	...
	name := cmd.Name
	if projection.NormalizeCommandType(cmd.Type) != projection.CommandTypeSlash {
		name = strings.TrimSpace(cmd.Name)
	}
	var options []discord.ApplicationCommandOption
	if projection.NormalizeCommandType(cmd.Type) == projection.CommandTypeSlash && (len(cmd.Subcommands) > 0 || len(cmd.Groups) > 0) {
	...
	} else if projection.NormalizeCommandType(cmd.Type) == projection.CommandTypeSlash {
	...
	switch projection.NormalizeCommandType(cmd.Type) {
...
func buildGroups(
	pluginID string,
	groups []projection.CommandGroup,
	locales []string,
	localize CommandLocalizer,
) []discord.ApplicationCommandOption {
...
func normalizeRequiredOptionsFirst(opts []projection.CommandOption) []projection.CommandOption {
	if len(opts) < 2 {
		return opts
	}
	...
	required := make([]projection.CommandOption, 0, len(opts))
	optional := make([]projection.CommandOption, 0, len(opts))
	for _, opt := range opts {
		if opt.Required {
...
func buildStringChoices(in []projection.OptionChoice) []discord.ApplicationCommandOptionChoiceString {
...
func buildIntChoices(in []projection.OptionChoice) []discord.ApplicationCommandOptionChoiceInt {
...
func buildFloatChoices(in []projection.OptionChoice) []discord.ApplicationCommandOptionChoiceFloat {
