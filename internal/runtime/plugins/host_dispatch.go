...
func (host *Host) PlanCommand(kind, name string, path []string) (InvocationPlan, error) {
	...
	definition, found := findCommandDefinition(plugin.Definition, contract.CommandKind(NormalizeCommandType(kind)), path)
	if !found {
...
func (host *Host) invokePrepared(ctx context.Context, plugin *Plugin, invocation contract.Invocation) (contract.Outcome, error) {
	...
	return plugin.Runtime.Invoke(ctx, invocation, starlarkruntime.InvocationServices{Localizer: services, Reader: services, Resources: services, HTTP: host.http, HTTPHosts: append([]string(nil), plugin.Manifest.Permissions.Network.Hosts...), Capabilities: append([]contract.Capability(nil), plugin.Capabilities...)}, host.pluginPrint(plugin.ID))
}
...
func (admission *Admission) runPrepared(ctx context.Context, responseState contract.ResponseState, allowRefresh bool) (contract.Operation, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		if err != nil {
			if allowRefresh && starlarkruntime.IsErrorKind(err, starlarkruntime.ErrorStale) {
				return admission.readmit(ctx, responseState)
			}
			return nil, err
		}
...
func (host *Host) loadState(ctx context.Context, plugin *Plugin, guildIDText string) ([]contract.StateEntry, error) {
	...
	versioned, ok := host.store.PluginKV().(store.VersionedPluginKVStore)
	if !ok {
...
func (host *Host) authorizeInvocation(ctx context.Context, plugin *Plugin, invocation contract.Invocation) (contract.Operation, bool, error) {
	...
	for _, check := range checks {
		...
		decision, err := plugin.Runtime.Check(ctx, checkInvocation, starlarkruntime.InvocationServices{Localizer: services, Reader: services, Resources: services, HTTP: host.http, HTTPHosts: append([]string(nil), plugin.Manifest.Permissions.Network.Hosts...), Capabilities: append([]contract.Capability(nil), plugin.Capabilities...)}, host.pluginPrint(plugin.ID))
		if err != nil {
...
func (host *Host) executeStorageEffect(ctx context.Context, invocation contract.Invocation, operation contract.Operation) error {
	...
	case *contract.CreateCheckInOperation:
		...
		return host.store.CheckIns().CreateCheckIn(ctx, store.CheckIn{ID: uuid.NewString(), UserID: authorID, Mood: value.Mood, CreatedAt: time.Unix(value.CreatedAt, 0).UTC()})
	case *contract.CreateReminderOperation:
		if authorErr != nil {
			return authorErr
		}
		reminder := store.Reminder{ID: value.ReminderID, UserID: authorID, Schedule: value.Schedule, Kind: value.Kind, Note: value.Note, Delivery: store.ReminderDelivery(value.Delivery), Enabled: true, NextRunAt: time.Unix(value.NextRunAt, 0).UTC(), CreatedAt: time.Unix(invocation.NowUnix, 0).UTC(), UpdatedAt: time.Unix(invocation.NowUnix, 0).UTC()}
		if value.ChannelID != "" {
	...
	case *contract.CreateWarningOperation:
		...
		return host.store.Warnings().CreateWarning(ctx, store.Warning{ID: uuid.NewString(), GuildID: guildID, UserID: target, ModeratorID: authorID, Reason: value.Reason, CreatedAt: time.Unix(value.CreatedAt, 0).UTC()})
...
func (host *Host) appendAudit(ctx context.Context, invocation contract.Invocation, value *contract.AppendAuditOperation) error {
	entry := store.AuditEntry{Action: value.Action, CreatedAt: time.Unix(value.CreatedAt, 0).UTC(), MetaJSON: "{}"}
	if invocation.Guild != nil {
	...
	if value.TargetType != "" {
		...
		kind := store.TargetType(value.TargetType)
