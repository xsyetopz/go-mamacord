...
func (v *VM) CallHandle(ctx context.Context, funcName string, cmd string, payload Payload) (any, bool, error) {
	res, hasValue, err := v.callHandleResult(ctx, funcName, cmd, payload)
	if err != nil || !hasValue {
...
func (v *VM) CallEncodedHandle(ctx context.Context, funcName string, cmd string, payload Payload) (EncodedValue, bool, error) {
	res, hasValue, err := v.callHandleResult(ctx, funcName, cmd, payload)
	if err != nil || !hasValue {
		return nil, hasValue, err
	}

	encoded, err := encodeLuaResult(res, fmt.Sprintf("lua %q", funcName))
	if err != nil {
...
func (v *VM) callHandleResult(ctx context.Context, funcName string, cmd string, payload Payload) (lua.LValue, bool, error) {
	v.mu.Lock()
	defer v.mu.Unlock()
	...
	if fn.Type() != lua.LTFunction {
		return lua.LNil, false, fmt.Errorf("lua function %q not found", funcName)
	}
	...
	if callErr := v.L.PCall(pcallNArgs, pcallNRet, nil); callErr != nil {
		return lua.LNil, false, fmt.Errorf("lua call %q: %w", funcName, callErr)
	}
...
func (v *VM) luaT(l *lua.LState) int {
	...
	if v.userID != 0 {
		...
		if _, ok := data["Pet"]; !ok {
			data["Pet"] = persona.PetName(v.locale, v.userID, id)
		}
		if _, ok := data["Mommy"]; !ok {
			data["Mommy"] = persona.Mommy(v.locale)
		}
	}
