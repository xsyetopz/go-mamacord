...
func loadProductionStarlarkHost(t *testing.T, bridge DiscordBridge) *Host {
	...
	host, err := NewHost(Options{Dirs: []string{filepath.Join(root, "plugins")}, AllowUnsignedPlugin: true, PermissionsFile: filepath.Join(root, "config", "permissions.json"), TrustedKeysFile: filepath.Join(root, "config", "trusted_keys.json"), Bridge: Bridge{Discord: bridge}, Logger: slog.New(slog.NewTextHandler(io.Discard, nil)), I18n: &registry})
	if err != nil {
		t.Fatal(err)
	}
...
func TestProductionStarlarkHostLoadsAndInvokesFun(t *testing.T) {
	...
	terminal, err := host.Run(context.Background(), "fun", contract.Invocation{Route: plan.Route, Kind: contract.InvocationCommand, ResponseState: contract.ResponseUnacknowledged, Author: &contract.UserRef{ID: "175928847299117063", Username: "tester", Name: "Tester"}, Locale: "en-US", Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"flip"}}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestProductionStarlarkHostEnforcesCommandPermissionsBeforeEffects(t *testing.T) {
	...
	invocation := contract.Invocation{Route: plan.Route, Kind: contract.InvocationCommand, ResponseState: contract.ResponseDeferredCreate, Guild: &contract.GuildRef{ID: "175928847299117063", Name: "Guild"}, Channel: &contract.ChannelRef{ID: "175928847299117064", GuildID: "175928847299117063", Kind: contract.ChannelText}, Author: &contract.UserRef{ID: "175928847299117065"}, Member: &contract.MemberRef{GuildID: "175928847299117063", User: contract.UserRef{ID: "175928847299117065"}}, Locale: "en-US", Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"slowmode"}, Options: []contract.OptionValue{{Name: "seconds", Kind: contract.OptionInteger, Integer: 0}}}}
...
func (kv *conflictingKV) GetPluginKVVersioned(context.Context, uint64, string, string) (store.PluginKVValue, bool, error) {
	kv.mu.Lock()
	defer kv.mu.Unlock()
	kv.reads++
	return store.PluginKVValue{ValueJSON: kv.value, Version: kv.version}, kv.exists, nil
}
...
func TestProductionStarlarkHostRetriesVersionedStateConflict(t *testing.T) {
	...
	kv := &conflictingKV{}
	host, err := NewHost(Options{Dirs: []string{filepath.Join(root, "examples", "plugins")}, AllowUnsignedPlugin: true, PermissionsFile: filepath.Join(root, "config", "permissions.json"), Store: hostStoreStub{kv: kv}, Logger: slog.New(slog.NewTextHandler(io.Discard, nil)), I18n: &registry})
	if err != nil {
		t.Fatal(err)
	}
	if err = host.LoadAll(context.Background()); err != nil {
	...
	terminal, err := host.Run(context.Background(), "example", contract.Invocation{Route: plan.Route, Kind: contract.InvocationCommand, ResponseState: contract.ResponseUnacknowledged, Guild: &contract.GuildRef{ID: "175928847299117063"}, Channel: &contract.ChannelRef{ID: "175928847299117064", GuildID: "175928847299117063", Kind: contract.ChannelText}, Author: &contract.UserRef{ID: "175928847299117065"}, Locale: "en-US", Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"example"}}})
	if err != nil {
		t.Fatal(err)
	}
	message, ok := terminal.(*contract.MessageOperation)
...
func writeCustomCheckPlugin(t *testing.T, root string) {
	...
	manifest, err := json.Marshal(StarlarkManifest{Schema: StarlarkManifestSchema, ID: "checks", Name: "Checks", Version: "0.1.0", Entrypoint: StarlarkEntrypoint, Permissions: StarlarkManifestPermissions{Network: StarlarkManifestNetworkPermissions{Hosts: []string{}}}, Locales: StarlarkManifestLocales{Default: "en-US", Supported: []string{"en-US"}}, StateKeys: []string{}, Assets: []string{}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestHostCustomCheckDenialsRespectOriginalRouteProtocol(t *testing.T) {
	...
	host, err := NewHost(Options{Dirs: []string{dir}, AllowUnsignedPlugin: true, Logger: slog.New(slog.NewTextHandler(io.Discard, nil))})
	if err != nil {
		t.Fatal(err)
	}
	if err = host.LoadAll(context.Background()); err != nil {
	...
	base := contract.Invocation{Author: &contract.UserRef{ID: "175928847299117063"}}
	command := base
	...
	autocomplete.Autocomplete = &contract.AutocompleteInput{Path: []string{"checked"}, Option: "value", Focused: contract.OptionValue{Name: "value", Kind: contract.OptionInteger, Integer: 1}}
...
func TestProductionManagerRoleGroupPermissionIsEnforcedAtRuntime(t *testing.T) {
	...
	invocation := contract.Invocation{Route: plan.Route, Kind: contract.InvocationCommand, ResponseState: contract.ResponseDeferredCreate, Guild: &contract.GuildRef{ID: "175928847299117063"}, Channel: &contract.ChannelRef{ID: "175928847299117064", GuildID: "175928847299117063", Kind: contract.ChannelText}, Author: &contract.UserRef{ID: "175928847299117065"}, Member: &contract.MemberRef{GuildID: "175928847299117063", User: contract.UserRef{ID: "175928847299117065"}}, Locale: "en-US", Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"roles", "delete"}, Options: []contract.OptionValue{{Name: "role", Kind: contract.OptionRole, Role: &contract.RoleRef{ID: "175928847299117066", GuildID: "175928847299117063", Name: "old", Mention: "<@&175928847299117066>"}}}}}
	terminal, err := host.Run(context.Background(), "manager", invocation)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAdmissionRunsOnceAfterAuthorization(t *testing.T) {
	...
	host, err := NewHost(Options{Dirs: []string{root}, AllowUnsignedPlugin: true, Logger: slog.New(slog.NewTextHandler(io.Discard, nil))})
	if err != nil {
		t.Fatal(err)
	}
	if err = host.LoadAll(context.Background()); err != nil {
	...
	invocation := contract.Invocation{Route: plan.Route, Kind: contract.InvocationCommand, ResponseState: contract.ResponseUnacknowledged, Author: &contract.UserRef{ID: "175928847299117063"}, Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"test"}}}
...
func TestStaticPermissionDenialPrecedesCustomChecks(t *testing.T) {
	...
	invocation := contract.Invocation{Route: plan.Route, Kind: contract.InvocationCommand, ResponseState: contract.ResponseUnacknowledged, Guild: &contract.GuildRef{ID: "175928847299117063"}, Author: &contract.UserRef{ID: "175928847299117064"}, Member: &contract.MemberRef{GuildID: "175928847299117063", User: contract.UserRef{ID: "175928847299117064"}}, Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"protected"}}}
