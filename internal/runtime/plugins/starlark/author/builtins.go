...
func AuthorAPI() starlarkgo.StringDict {
	...
	for name, value := range effects.API() {
		api[name] = value
	}
	return api
}
...
func commandBuiltin(kind string) builtinFunc {
	return func(_ *starlarkgo.Thread, builtin *starlarkgo.Builtin, args starlarkgo.Tuple, kwargs []starlarkgo.Tuple) (starlarkgo.Value, error) {
		...
		value := commandDeclaration{commandIdentity: commandIdentity{kind: kind, id: id, name: name, description: description, descriptionID: descriptionID}, commandBehavior: commandBehavior{handler: handler, ephemeral: ephemeral, deferMode: deferMode}}
		var err error
...
func contextCommandBuiltin(kind string) builtinFunc {
	return func(_ *starlarkgo.Thread, builtin *starlarkgo.Builtin, args starlarkgo.Tuple, kwargs []starlarkgo.Tuple) (starlarkgo.Value, error) {
		...
		value := commandDeclaration{commandIdentity: commandIdentity{kind: kind, id: id, name: name}, commandBehavior: commandBehavior{handler: handler, ephemeral: ephemeral, deferMode: deferMode}}
		var err error
		if value.permissions, err = stringList(permissions); err != nil {
			return nil, fmt.Errorf("permissions: %w", err)
		}
...
func groupBuiltin(kind string) builtinFunc {
	return func(_ *starlarkgo.Thread, builtin *starlarkgo.Builtin, args starlarkgo.Tuple, kwargs []starlarkgo.Tuple) (starlarkgo.Value, error) {
		...
		value := commandDeclaration{commandIdentity: commandIdentity{kind: kind, name: name, description: description, descriptionID: descriptionID}}
		var err error
...
func builtinOption(_ *starlarkgo.Thread, _ *starlarkgo.Builtin, args starlarkgo.Tuple, kwargs []starlarkgo.Tuple) (starlarkgo.Value, error) {
	...
	value := optionDeclaration{optionIdentity: optionIdentity{kind: kind, name: name, description: description, descriptionID: descriptionID, required: required}, optionChoices: optionChoices{autocomplete: autocomplete}}
	var err error
...
func apiValueList(value starlarkgo.Value, kind apiValueKind) ([]*apiValue, error) {
	...
	for iterator.Next(&item) {
		...
		if len(out) > evaluation.MaxCollectionItems {
