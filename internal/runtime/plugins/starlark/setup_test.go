...
ENTRY_MUTABLE = []
`
	bundle := &memoryBundle{sources: map[string][]byte{EntrypointLabel: []byte(source)}}
	...
	if len(generation.routes) != 6 {
		t.Fatalf("routes = %d, want 6", len(generation.routes))
	}
	if _, err := generation.catalog.Resolve(contract.Invocation{PluginID: "test", Generation: generation.ID(), Route: leaf.Route, Kind: contract.InvocationCommand, ResponseState: contract.ResponseUnacknowledged, Author: &contract.UserRef{ID: "1"}, Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"tool", "create"}, Options: []contract.OptionValue{{Name: "text", Kind: contract.OptionString, String: "hello"}}}}); err != nil {
...
`, contains: "exactly one"},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			compiled, err := CompileBundle(context.Background(), &memoryBundle{sources: map[string][]byte{EntrypointLabel: []byte(test.source)}}, DefaultLimits())
			if err != nil {
				t.Fatalf("CompileBundle: %v", err)
			}
			initialized, err := compiled.Initialize(context.Background(), AuthorAPI(), nil)
			...
			_, err = initialized.Setup(context.Background(), "generation-1", nil)
