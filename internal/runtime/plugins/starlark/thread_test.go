...
func TestThreadExecutionClassifiesLimits(t *testing.T) {
	...
	step := newThreadExecution(context.Background(), "step", 10, time.Second, 0, 0, nil)
	...
	deadline := newThreadExecution(context.Background(), "deadline", 100, time.Millisecond, 0, 0, nil)
	...
	cancel()
	caller := newThreadExecution(parent, "caller", 100, time.Second, 0, 0, nil)
