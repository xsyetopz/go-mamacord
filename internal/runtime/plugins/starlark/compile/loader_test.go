...
func TestCompileAndInitializeControlledModules(t *testing.T) {
	...
	bundle := &memoryBundle{sources: map[string][]byte{
		evaluation.EntrypointLabel: []byte("load(\"@mamacord//api.star\", \"identity\")\nload(\"//lib:values.star\", \"VALUES\")\nRESULT = identity(VALUES[0])\nENTRY = []\n"),
		"//lib:values.star":        []byte("VALUES = [42]\n"),
	}}
	compiled, err := CompileBundle(context.Background(), bundle, evaluation.DefaultLimits())
	if err != nil {
	...
	if len(bundle.reads) != 2 || bundle.reads[0] != evaluation.EntrypointLabel || bundle.reads[1] != "//lib:values.star" {
...
func TestCompileBundleRejectsGraphViolations(t *testing.T) {
	...
	tests := []struct {
		name    string
		sources map[string][]byte
		limits  func() evaluation.Limits
		kind    evaluation.ErrorKind
	}{
		{name: "relative load", sources: map[string][]byte{evaluation.EntrypointLabel: []byte("load(\"lib.star\", \"x\")\n")}, limits: evaluation.DefaultLimits, kind: evaluation.ErrorLoad},
		{name: "cycle", sources: map[string][]byte{evaluation.EntrypointLabel: []byte("load(\"//lib:a.star\", \"a\")\n"), "//lib:a.star": []byte("load(\"//lib:b.star\", \"b\")\na = b\n"), "//lib:b.star": []byte("load(\"//lib:a.star\", \"a\")\nb = a\n")}, limits: evaluation.DefaultLimits, kind: evaluation.ErrorLoad},
		{name: "module count", sources: map[string][]byte{evaluation.EntrypointLabel: []byte("load(\"//lib:a.star\", \"a\")\n"), "//lib:a.star": []byte("a = 1\n")}, limits: func() evaluation.Limits { value := evaluation.DefaultLimits(); value.MaxModules = 1; return value }, kind: evaluation.ErrorLoad},
		{name: "load depth", sources: map[string][]byte{evaluation.EntrypointLabel: []byte("load(\"//lib:a.star\", \"a\")\n"), "//lib:a.star": []byte("a = 1\n")}, limits: func() evaluation.Limits { value := evaluation.DefaultLimits(); value.MaxLoadDepth = 1; return value }, kind: evaluation.ErrorLoad},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := CompileBundle(context.Background(), &memoryBundle{sources: test.sources}, test.limits())
			if err == nil || !evaluation.IsErrorKind(err, test.kind) {
...
func TestInitializePreservesLoadErrorKind(t *testing.T) {
	t.Parallel()
	bundle := &memoryBundle{sources: map[string][]byte{
		evaluation.EntrypointLabel: []byte("load(\"//lib:broken.star\", \"VALUE\")\n"),
	...
	if err == nil || !evaluation.IsErrorKind(err, evaluation.ErrorLoad) {
...
func TestInitializeUsesOneFrozenModuleInstance(t *testing.T) {
	t.Parallel()
	bundle := &memoryBundle{sources: map[string][]byte{
		evaluation.EntrypointLabel: []byte("load(\"//lib:left.star\", \"LEFT\")\nload(\"//lib:right.star\", \"RIGHT\")\n"),
...
func TestCompileBundleHonorsCancellationAndAggregateBytes(t *testing.T) {
	...
	cancel()
	_, err := CompileBundle(ctx, &memoryBundle{sources: map[string][]byte{evaluation.EntrypointLabel: []byte("x = 1\n")}}, evaluation.DefaultLimits())
	...
	library := "a = 1\n" + strings.Repeat("#", 40)
	_, err = CompileBundle(context.Background(), &memoryBundle{sources: map[string][]byte{evaluation.EntrypointLabel: []byte(main), "//lib:a.star": []byte(library)}}, limits)
	if err == nil || !evaluation.IsErrorKind(err, evaluation.ErrorSource) {
