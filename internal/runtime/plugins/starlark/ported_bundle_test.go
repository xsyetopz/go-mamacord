...
func TestPortedBundlesCompileAndInitialize(t *testing.T) {
	...
	for name, bundlePath := range paths {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			bundle, err := OpenDirBundle(bundlePath)
			if err != nil {
				t.Fatal(err)
			}
			...
			generation, err := initialized.Setup(context.Background(), contract.GenerationID(name+"-test"), nil)
			if err != nil {
				t.Fatal(err)
			}
...
func (exampleLocalizer) Localize(_ context.Context, request LocalizationRequest) (string, error) {
...
func loadPortedExample(t *testing.T) *Generation {
	t.Helper()
	bundle, err := OpenDirBundle(filepath.Join("..", "..", "..", "..", "examples", "plugins", "example", "bundles", "example-v0.1.0"))
	if err != nil {
		t.Fatal(err)
	}
	compiled, err := CompileBundle(context.Background(), bundle, DefaultLimits())
	...
	generation, err := initialized.Setup(context.Background(), "example-test", nil)
	if err != nil {
		t.Fatal(err)
	}
	return generation
...
func TestPortedExampleCounterUsesVersionedStateEffects(t *testing.T) {
	...
	services := InvocationServices{Localizer: exampleLocalizer{}, Capabilities: []contract.Capability{contract.CapabilityStorageKV}}
	base := contract.Invocation{PluginID: "example", Generation: generation.ID(), ResponseState: contract.ResponseUnacknowledged, Guild: &contract.GuildRef{ID: "10"}, Channel: &contract.ChannelRef{ID: "20", GuildID: "10", Kind: contract.ChannelText}, Author: &contract.UserRef{ID: "30"}, Locale: "en-US", State: []contract.StateEntry{{Key: "counter", Value: contract.IntValue(2), Version: 7}}}
	command := base
...
func (passthroughLocalizer) Localize(_ context.Context, request LocalizationRequest) (string, error) {
	return request.MessageID, nil
}
func loadPortedFun(t *testing.T) *Generation {
	t.Helper()
	bundle, err := OpenDirBundle(filepath.Join("..", "..", "..", "..", "plugins", "fun", "bundles", "release-v0.1.0"))
	if err != nil {
		t.Fatal(err)
	}
	compiled, err := CompileBundle(context.Background(), bundle, DefaultLimits())
	...
	generation, err := initialized.Setup(context.Background(), "fun-test", nil)
	if err != nil {
		t.Fatal(err)
	}
	return generation
...
func TestPortedFunIsDeterministicAndUsesBoundedHTTP(t *testing.T) {
	...
	base := contract.Invocation{PluginID: "fun", Generation: generation.ID(), Kind: contract.InvocationCommand, ResponseState: contract.ResponseUnacknowledged, Guild: &contract.GuildRef{ID: "10"}, Author: &contract.UserRef{ID: "30"}, Locale: "en-US", RandomSeed: 99}
	roll := base
	roll.Route = "command:slash:roll"
	roll.Command = &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"roll"}, Options: []contract.OptionValue{{Name: "number", Kind: contract.OptionInteger, Integer: 3}, {Name: "sides", Kind: contract.OptionInteger, Integer: 6}, {Name: "modifier", Kind: contract.OptionInteger, Integer: 1}}}
	services := InvocationServices{Localizer: passthroughLocalizer{}}
	...
	hug.Command = &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"hug"}, Options: []contract.OptionValue{{Name: "user", Kind: contract.OptionUser, User: &contract.UserRef{ID: "40"}}}}
	services = InvocationServices{Localizer: passthroughLocalizer{}, HTTP: client, HTTPHosts: []string{"kawaii.red"}, Capabilities: []contract.Capability{contract.CapabilityNetworkHTTP}}
...
func loadPortedInfo(t *testing.T) *Generation {
	t.Helper()
	bundle, err := OpenDirBundle(filepath.Join("..", "..", "..", "..", "plugins", "info", "bundles", "release-v0.1.0"))
	if err != nil {
		t.Fatal(err)
	}
	compiled, err := CompileBundle(context.Background(), bundle, DefaultLimits())
	...
	generation, err := initialized.Setup(context.Background(), "info-test", nil)
	if err != nil {
		t.Fatal(err)
	}
	return generation
...
func TestPortedInfoUsesReadersOnlyForDiscordReads(t *testing.T) {
	...
	services := InvocationServices{Reader: reader, Localizer: passthroughLocalizer{}, Capabilities: []contract.Capability{contract.CapabilityDiscordUsers, contract.CapabilityDiscordMembers}}
	invocation := contract.Invocation{PluginID: "info", Generation: generation.ID(), Route: "command:slash:lookup.user", Kind: contract.InvocationCommand, ResponseState: contract.ResponseDeferredCreate, Guild: &contract.GuildRef{ID: "10"}, Author: &contract.UserRef{ID: "30"}, Locale: "en-US", Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"lookup", "user"}, Options: []contract.OptionValue{{Name: "user", Kind: contract.OptionUser, User: &contract.UserRef{ID: "40"}}}}}
	...
	roleInvocation := contract.Invocation{PluginID: "info", Generation: generation.ID(), Route: "command:slash:lookup.role", Kind: contract.InvocationCommand, ResponseState: contract.ResponseUnacknowledged, Guild: &contract.GuildRef{ID: "10"}, Author: &contract.UserRef{ID: "30"}, Locale: "en-US", Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"lookup", "role"}, Options: []contract.OptionValue{{Name: "role", Kind: contract.OptionRole, Role: &contract.RoleRef{ID: "50", GuildID: "10", Name: "Helper", Mention: "<@&50>", CreatedAt: 100}}}}}
	outcome, err = generation.InvokeWithServices(context.Background(), roleInvocation, InvocationServices{Localizer: passthroughLocalizer{}}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func loadPortedManager(t *testing.T) *Generation {
	t.Helper()
	bundle, err := OpenDirBundle(filepath.Join("..", "..", "..", "..", "plugins", "manager", "bundles", "release-v0.1.0"))
	if err != nil {
		t.Fatal(err)
	}
	compiled, err := CompileBundle(context.Background(), bundle, DefaultLimits())
	...
	generation, err := initialized.Setup(context.Background(), "manager-test", nil)
	if err != nil {
		t.Fatal(err)
	}
	return generation
...
func TestPortedManagerReturnsGuardedAuthorizedEffects(t *testing.T) {
	...
	invocation := contract.Invocation{PluginID: "manager", Generation: generation.ID(), Route: "command:slash:slowmode", Kind: contract.InvocationCommand, ResponseState: contract.ResponseDeferredCreate, Guild: &contract.GuildRef{ID: "10"}, Channel: &contract.ChannelRef{ID: "20", GuildID: "10", Kind: contract.ChannelText}, Author: &contract.UserRef{ID: "30"}, Locale: "en-US", Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"slowmode"}}}
	services := InvocationServices{Localizer: passthroughLocalizer{}, Capabilities: []contract.Capability{contract.CapabilityDiscordChannels}}
	outcome, err := generation.InvokeWithServices(context.Background(), invocation, services, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := generation.InvokeWithServices(context.Background(), invocation, InvocationServices{Localizer: passthroughLocalizer{}}, nil); err == nil || !IsErrorKind(err, ErrorValidation) {
...
func loadPortedModeration(t *testing.T) *Generation {
	t.Helper()
	bundle, err := OpenDirBundle(filepath.Join("..", "..", "..", "..", "plugins", "moderation", "bundles", "release-v0.1.0"))
	if err != nil {
		t.Fatal(err)
	}
	compiled, err := CompileBundle(context.Background(), bundle, DefaultLimits())
	...
	generation, err := initialized.Setup(context.Background(), "moderation-test", nil)
	if err != nil {
		t.Fatal(err)
	}
	return generation
...
func TestPortedModerationReturnsTransactionalFailurePolicies(t *testing.T) {
	...
	invocation := contract.Invocation{PluginID: "moderation", Generation: generation.ID(), Route: "command:slash:warn", Kind: contract.InvocationCommand, ResponseState: contract.ResponseDeferredCreate, Guild: &contract.GuildRef{ID: "10"}, Channel: &contract.ChannelRef{ID: "20", GuildID: "10", Kind: contract.ChannelText}, Author: &contract.UserRef{ID: "30"}, Locale: "en-US", NowUnix: 1000, State: []contract.StateEntry{{Key: "guild_config", Value: config, Version: 1}}, Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"warn"}, Options: []contract.OptionValue{{Name: "user", Kind: contract.OptionUser, User: &contract.UserRef{ID: "40", Name: "Target"}}, {Name: "reason", Kind: contract.OptionString, String: "Be kind"}}}}
	services := InvocationServices{Reader: &stubReader{}, Localizer: passthroughLocalizer{}, Capabilities: []contract.Capability{contract.CapabilityStorageKV, contract.CapabilityStorageWarnings, contract.CapabilityStorageAudit, contract.CapabilityDiscordMessages, contract.CapabilityDiscordMembers}}
	outcome, err := generation.InvokeWithServices(context.Background(), invocation, services, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func loadPortedWellness(t *testing.T) *Generation {
	t.Helper()
	bundle, err := OpenDirBundle(filepath.Join("..", "..", "..", "..", "plugins", "wellness", "bundles", "release-v0.1.0"))
	if err != nil {
		t.Fatal(err)
	}
	compiled, err := CompileBundle(context.Background(), bundle, DefaultLimits())
	...
	generation, err := initialized.Setup(context.Background(), "wellness-test", nil)
	if err != nil {
		t.Fatal(err)
	}
	return generation
...
func TestPortedWellnessCreatesDeterministicReminderEffect(t *testing.T) {
	...
	services := InvocationServices{Reader: reader, Localizer: passthroughLocalizer{}, Capabilities: []contract.Capability{contract.CapabilityStorageReminders}}
	invocation := contract.Invocation{PluginID: "wellness", Generation: generation.ID(), Route: "command:slash:remind.create", Kind: contract.InvocationCommand, ResponseState: contract.ResponseDeferredCreate, Author: &contract.UserRef{ID: "30"}, Locale: "en-US", NowUnix: 1000, RandomSeed: 123, Command: &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"remind", "create"}, Options: []contract.OptionValue{{Name: "schedule", Kind: contract.OptionString, String: "0 9 * * *"}, {Name: "kind", Kind: contract.OptionString, String: "hydrate"}}}}
...
func TestOfficialDefinitionsRetainLocalizedDescriptionsAndManagerRolePermission(t *testing.T) {
	...
	for _, name := range []string{"fun", "info", "manager", "moderation", "wellness"} {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			bundle, err := OpenDirBundle(filepath.Join("..", "..", "..", "..", "plugins", name, "bundles", "release-v0.1.0"))
			if err != nil {
				t.Fatal(err)
			}
			compiled, err := CompileBundle(context.Background(), bundle, DefaultLimits())
			...
			generation, err := initialized.Setup(context.Background(), contract.GenerationID(name+"-descriptions"), nil)
			if err != nil {
				t.Fatal(err)
			}
...
func TestPortedModerationDisabledConfigCoversEveryRoute(t *testing.T) {
	...
	base := contract.Invocation{PluginID: "moderation", Generation: generation.ID(), Guild: &contract.GuildRef{ID: "10"}, Channel: &contract.ChannelRef{ID: "20", GuildID: "10", Kind: contract.ChannelText}, Author: &contract.UserRef{ID: "30"}, Locale: "en-US", NowUnix: 1000, State: []contract.StateEntry{{Key: "guild_config", Value: disabled, Version: 1}}}
	...
	tests[0].Command = &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"warn"}, Options: []contract.OptionValue{{Name: "user", Kind: contract.OptionUser, User: &contract.UserRef{ID: "40"}}, {Name: "reason", Kind: contract.OptionString, String: "reason"}}}
	...
	tests[1].Command = &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"unwarn"}, Options: []contract.OptionValue{{Name: "user", Kind: contract.OptionUser, User: &contract.UserRef{ID: "40"}}}}
	...
	tests[2].Component = &contract.ComponentInput{ID: "unwarn_select", Kind: contract.ComponentStringSelect, Values: []contract.OptionValue{{Name: "value", Kind: contract.OptionString, String: "warning|30|40|1000"}}}
	for _, invocation := range tests {
		outcome, err := generation.InvokeWithServices(context.Background(), invocation, InvocationServices{Localizer: passthroughLocalizer{}, Capabilities: []contract.Capability{contract.CapabilityStorageKV}}, nil)
		if err != nil {
...
func TestPortedManagerRejectsKnownInvalidMediaBeforeEffects(t *testing.T) {
	generation := loadPortedManager(t)
	base := contract.Invocation{PluginID: "manager", Generation: generation.ID(), Kind: contract.InvocationCommand, ResponseState: contract.ResponseDeferredCreate, Guild: &contract.GuildRef{ID: "10"}, Channel: &contract.ChannelRef{ID: "20", GuildID: "10", Kind: contract.ChannelText}, Author: &contract.UserRef{ID: "30"}, Locale: "en-US"}
	create := base.DeepClone()
	create.Route = "command:slash:emojis.create"
	create.Command = &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"emojis", "create"}, Options: []contract.OptionValue{{Name: "name", Kind: contract.OptionString, String: "wave"}, {Name: "file", Kind: contract.OptionAttachment, Attachment: &contract.AttachmentRef{ID: "50", Filename: "wave.txt", ContentType: "text/plain", URL: "https://cdn.discordapp.com/wave.txt", Size: 10, Width: 32, Height: 32}}}}
	outcome, err := generation.InvokeWithServices(context.Background(), create, InvocationServices{Localizer: passthroughLocalizer{}}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if len(outcome.Operations) != 1 {
	...
	edit.Command = &contract.CommandInput{Kind: contract.CommandSlash, Path: []string{"emojis", "edit"}, Options: []contract.OptionValue{{Name: "emoji", Kind: contract.OptionString, String: "not-an-emoji"}, {Name: "name", Kind: contract.OptionString, String: "wave"}}}
	outcome, err = generation.InvokeWithServices(context.Background(), edit, InvocationServices{Localizer: passthroughLocalizer{}}, nil)
	if err != nil {
		t.Fatal(err)
	}
	response = outcome.Operations[0].(*contract.EditResponseOperation)
