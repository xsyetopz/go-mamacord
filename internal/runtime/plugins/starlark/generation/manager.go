...
func NewBuilder(print func(string)) GenerationBuilder {
	return GenerationBuilder{Limits: evaluation.DefaultLimits(), Print: print}
}

func IsStale(err error) bool {
	return evaluation.IsErrorKind(err, evaluation.ErrorStale)
}

func (builder GenerationBuilder) Build(ctx context.Context, source compile.SourceBundle, id contract.GenerationID) (*Generation, error) {
	compiled, err := compile.CompileBundle(ctx, source, builder.Limits)
	if err != nil {
		return nil, err
	}
	initialized, err := compiled.Initialize(ctx, author.AuthorAPI(), builder.Print)
	if err != nil {
		return nil, err
	}
	return BuildInitialized(ctx, initialized, id, builder.Print)
}
...
func (manager *GenerationManager) Invoke(ctx context.Context, invocation contract.Invocation, services contextapi.InvocationServices, print func(string)) (contract.Outcome, error) {
	if manager == nil {
		return contract.Outcome{}, evaluation.NewRuntimeError(evaluation.ErrorStale, "invoke", string(invocation.Route), errors.New("generation manager is nil"))
	}
	for attempt := 0; attempt < 2; attempt++ {
		generation := manager.Current()
		if generation == nil {
			return contract.Outcome{}, evaluation.NewRuntimeError(evaluation.ErrorStale, "invoke", string(invocation.Route), errors.New("no active generation"))
		}
		invocation.Generation = generation.ID()
		outcome, err := generation.InvokeWithServices(ctx, invocation, services, print)
		if !evaluation.IsErrorKind(err, evaluation.ErrorStale) || manager.Current() == generation {
			return outcome, err
		}
	}
	return contract.Outcome{}, evaluation.NewRuntimeError(evaluation.ErrorStale, "invoke", string(invocation.Route), errors.New("active generation changed during dispatch"))
}
func (manager *GenerationManager) Check(ctx context.Context, invocation contract.Invocation, services contextapi.InvocationServices, print func(string)) (contract.CheckDecision, error) {
	if manager == nil {
		return contract.CheckDecision{}, evaluation.NewRuntimeError(evaluation.ErrorStale, "check", string(invocation.Route), errors.New("generation manager is nil"))
	}
	for attempt := 0; attempt < 2; attempt++ {
		generation := manager.Current()
		if generation == nil {
			return contract.CheckDecision{}, evaluation.NewRuntimeError(evaluation.ErrorStale, "check", string(invocation.Route), errors.New("no active generation"))
		}
		invocation.Generation = generation.ID()
	...
	return contract.CheckDecision{}, evaluation.NewRuntimeError(evaluation.ErrorStale, "check", string(invocation.Route), errors.New("active generation changed during dispatch"))
}
