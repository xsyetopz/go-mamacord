...
func (services invocationRuntimeServices) Localize(ctx context.Context, request starlarkruntime.LocalizationRequest) (string, error) {
	if services.plugin == nil {
...
func reminderContract(item store.Reminder) contract.ReminderRef {
	out := contract.ReminderRef{ID: item.ID, Schedule: item.Schedule, Kind: item.Kind, Note: item.Note, Delivery: string(item.Delivery), Enabled: item.Enabled, NextRunAt: item.NextRunAt.UTC().Unix(), FailureCount: item.FailureCount, CreatedAt: item.CreatedAt.UTC().Unix(), UpdatedAt: item.UpdatedAt.UTC().Unix()}
	if item.GuildID != nil {
