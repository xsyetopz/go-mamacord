...
func optionsFromContract(sources []contract.OptionDefinition) []CommandOption {
	...
	for index, source := range sources {
		option := CommandOption{Name: source.Name, Type: optionTypeString(source.Kind), Description: source.Description, DescriptionID: source.DescriptionID, Required: source.Required, Autocomplete: string(source.Autocomplete), MinLength: cloneIntPointer(source.MinLength), MaxLength: cloneIntPointer(source.MaxLength), ChannelTypes: channelTypeNumbers(source.ChannelKinds)}
