...
func (dispatcher *InteractionDispatcher) OnModal(e *events.ModalSubmitInteractionCreate) {
	dispatcher.interactions.Launch("modal", func() { dispatcher.runPluginModal(e) }, func() {
		_ = e.CreateMessage(interactions.NoticeMessage(interactions.KindWarning, "", "Mamacord is busy. Try again in a moment.", true))
	})
}
func (dispatcher *InteractionDispatcher) runPluginModal(e *events.ModalSubmitInteractionCreate) {
	...
	if d := dispatcher.policy.ModalCooldown(customID); d > 0 {
		if remaining, ok := dispatcher.cooldowns.Take(uint64(e.User().ID), cooldown.ModalCooldownKey(customID), d, time.Now()); !ok {
	...
	if guildID := e.GuildID(); guildID != nil {
		...
		if err != nil {
			dispatcher.pluginModalError(ctx, e, t, customID, contract.ResponseUnacknowledged, err)
			return
		}
		if !enabled {
	...
	if err != nil {
		dispatcher.pluginModalError(ctx, e, t, customID, state, err)
	}
}
...
func (dispatcher *InteractionDispatcher) pluginModalError(ctx context.Context, e *events.ModalSubmitInteractionCreate, t commandtext.Translator, customID string, state contract.ResponseState, err error) {
	dispatcher.incInteractionFailure()
	dispatcher.incPluginFailure()
	dispatcher.logger.ErrorContext(ctx, "plugin modal failed", slog.String("custom_id", customID), slog.String("err", err.Error()))
	if state == contract.ResponseUnacknowledged {
